  ...
  const renderContent = (content) =>
    content.split(/(\`\`\`[\s\S]*?\`\`\`)/g).map((part, index) => {
      if (part.startsWith("```")) {
        ...
        return (
          <div key={index} className="flex-col w-[320px] xl:w-full relative border border-white/15 mb-5 bg-gray-900 pt-10 rounded-2xl">
            <button onClick={() => handleCopyCode(code, index)} className="absolute top-2 right-2 z-10 bg-transparent text-white border border-white/15 px-3 py-1 text-sm rounded-full hover:bg-gray-300 transition">
      ...
      <div className="pt-[100px] pb-[500px] pl-3 pr-3 xl:pt-[200px] xl:pb-[300px] xl:pl-[300px] xl:pr-[300px]">
        <div>
          ...
          <div className="bg-transparent border border-white/15 rounded-3xl">
            <textarea type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Type your message..." className="bg-transparent text-white rounded-3xl w-full h-20 p-5 resize-none focus:outline-none" disabled={isLoading} />
        ...
        <div className="flex justify-center p-5">
          <button onClick={handleClearHistory} className="px-6 py-3 bg-red-500 text-white rounded-full hover:bg-red-600 transition">
            Clear History
          </button>
        </div>
      </div>
